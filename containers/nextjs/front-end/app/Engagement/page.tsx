...
export default function Engagment() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const user = await axios.get('http://leetreviews.com:8000/42/me', { headers })
        setAvatar(user.data.avatar)
        const url = `http://leetreviews.com:8000/42/getComments/?id=${id}`;
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      const url = `http://leetreviews.com:8000/42/comments/?id=${id}`;
